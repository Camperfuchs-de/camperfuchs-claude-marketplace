...
- **Next-Fahrzeugseiten und Query-Parameter (11.09.2026):** Detailseiten sind edge-cachebar (`s-maxage=3600`), die Query gehoert aber zum Cache-Schluessel. Links mit `?from=…&to=…` landen deshalb fast immer im kalten SSR (1,2–3 s statt 60–90 ms). Nur-Browser-Werte in den Hash legen (PR 2009); Cloudflare die Query NICHT ignorieren lassen, weil die Seite `router.query` serverseitig rendert. Details: Skill camperfuchs-frontend-feature-shippen.
